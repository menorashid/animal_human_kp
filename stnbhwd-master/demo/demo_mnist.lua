...
if use_stn then 
   ...
   -- paths.dofile('test_TPSGridGenerator.lua')
...
for epoch=1,30 do
   ...
   for batchidx = 1, datasetTrain:getNumBatches() do
      ...
      -- print (model:get(1):get(1):get(1));
      -- out=model:get(1):get(1):get(1):forward(inputs:cuda());
      -- print (out:size());

      -- print (model:get(1).output:size());
      -- out=model:get(1):get(1):get(2):forward(model:get(1).output);
      -- print (out:size());

      -- print (model:get(1):get(1):get(1).output:size());
      -- print (model:get(1):get(1):get(1):get(2).output:size());
