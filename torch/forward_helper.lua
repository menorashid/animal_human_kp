...
do  
	...
	function Forward_Helper:forward_gtWarp(td,net,inputs,batch_targets,human_labels,horse_labels,saveImage,euclideanLoss,mean_im,std_im,train_euclidean,gt_net,tps_layer)
		local batch_inputs_view;
	    if saveImage then
	        batch_inputs_view=inputs:double():clone();
	        if td.soumith_locnet then
	    ...
	    local gt_output=tps_layer:getGTOutput(human_labels,horse_labels);
	    -- print (gt_output:size())
	    -- print (inputs:size());
	    -- print (gt_net);
        local midoutputs=gt_net:forward{inputs,gt_output};
